...
class _GraphRetriever(BaseRetriever):
    def _candidate_records(self, question: Question) -> list[CorpusRecord]:
        ...
        # A supplied graph is a real candidate boundary. Without a graph, the
        # deterministic corpus fixture remains usable for local demonstrations.
...
class StaticKGRetriever(_GraphRetriever):
    ...
    def _graph_candidate_ids(self, question: Question) -> frozenset[str]:
        candidates = getattr(self.graph, "candidate_evidence_ids", None)
...
class TemporalKGRetriever(_GraphRetriever):
    ...
    def _graph_candidate_ids(self, question: Question) -> frozenset[str]:
        ...
        return frozenset(candidates(question.issuer, question.query, question.cutoff)) if callable(candidates) else frozenset()
